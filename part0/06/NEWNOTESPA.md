## Sequence Diagram for creating a new note in an example spa
...
```mermaid
    sequenceDiagram
        ...
        note right of browser: User types a new note: { "content": "My new note", "date": "2024-06-23" } and presses the save button

        note right of browser: JavaScript saves the note in the array "notes"
        note right of browser: JavaScript redraws the notes
        note right of browser: JavaScript sends the new note to the server

        browser ->> server: POST https://studies.cs.helsinki.fi/exampleapp/new_note_spa
        activate server
        note left of server: Server processes the new note and returns status code 201
        server -->> browser: success message: {message: "note created"}
        deactivate server
```
